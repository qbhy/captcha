...
class Captcha
{
     ...
     * @throws \Exception
     */
    public static function instance($codeKey, $minutes = 20)
    {
        if (!isset(static::$codeKeyInstances[$codeKey])) {
            ...
            static::$codeKeyInstances[$codeKey] = new static(
                ...
                CaptchaStore::instance($codeKey, $minutes),
                app('hash'),
    ...
    protected function text()
    {
        ...
        foreach (str_split($this->text) as $char) {
            $marginLeft =
                $this->textLeftPadding + ($i * ($this->image->width() - $this->textLeftPadding) / $this->length);
